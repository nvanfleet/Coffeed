...
#pragma mark - Boolean
	else if(!strcasecmp(comm,"AUTOT"))
	{
		wasFloat = FALSE;
		
		int tuning = -1;
		
		// if == 1 then start tuning
		if(boolValue == TRUE)
			tuning = 2;
		else
			tuning = FALSE;
		...
		if(readonly == FALSE)
		{
			// If tuning is already running, cancel it
			if(state.tuning == TRUE)
				tuning = FALSE;
			
			state.tuning = tuning;
			sprintf(responseString,"%d",tuning);
		}
		else
			sprintf(responseString,"%d",state.tuning);
	}
...
#pragma mark - Float values
	...
	{
		if(readonly != TRUE)
			conf.pgain = floatValue;
		else
			floatValue = conf.pgain;
	}
	else if(!strcasecmp(comm,"IGAIN"))
	{
		if(readonly != TRUE)
			conf.igain = floatValue;
		else
			floatValue = conf.igain;
	}
	else if(!strcasecmp(comm,"DGAIN"))
	{
		if(readonly != TRUE)
			conf.dgain = floatValue;
		else
			floatValue = conf.dgain;
	}
